...
if isfield(EKF_DI_params, 'reset_token')
    if EKF_DI_params.reset_token ~= last_reset_token
        new_params_loaded = true;
        last_reset_token = EKF_DI_params.reset_token;
    end
end
...
if reset || ~initialized || new_params_loaded
    ...
    x_hat = EKF_DI_params.x0;
    P = EKF_DI_params.P0;

    t_prev = t_now;
    initialized = true;

    next_gps_time = t_now + EKF_DI_params.gps_period;
    ...
    euler_out = x_hat(7:9);
    vel_out   = x_hat(4:6);
    pos_out = ekf_pos_output(x_hat(1:3));
    acc_n_out = zeros(3,1);
    xhat_out  = x_hat;
...
if t_now >= next_gps_time
    ...
    while next_gps_time <= t_now
        next_gps_time = next_gps_time + EKF_DI_params.gps_period;
    end
end
